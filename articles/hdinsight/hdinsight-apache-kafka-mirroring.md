...
>
> Hataya dayanıklılık hakkında endişeleriniz varsa, küme içinde çoğaltma konular için ayarlamanız gerekir. Daha fazla bilgi için bkz: [Kafka hdınsight kullanmaya başlama](hdinsight-apache-kafka-get-started.md).
...
Kaynakları oluşturduktan sonra kümeler ve web Pano içeren kaynak grubu için bir dikey penceresine yönlendirilirsiniz.

![Kaynak grubu dikey vnet ve kümeler için](./media/hdinsight-apache-kafka-mirroring/groupblade.png)

> [!IMPORTANT]
> Hdınsight kümeleri adlarının bildirimi **kaynak BASENAME** ve **taşınmaya BASENAME**, BASENAME şablona verdiğiniz adı olduğu. Kümeye bağlanırken bu adları daha sonraki adımlarda kullanın.
...
2. Oluşturmak için aşağıdaki komutu kullanın bir `consumer.properties` ile iletişim kurmak açıklar dosya **kaynak** küme:
...
3. Hedef küme ile iletişim kurar üretici yapılandırmadan önce ana bilgisayarlar için aracı bulmalıdır **hedef** küme. Bu bilgileri almak için aşağıdaki komutları kullanın:
    ...
    Değiştir `$PASSWORD` küme için oturum açma hesabı (Yönetici) parolayla.

    Değiştir `$CLUSTERNAME` hedef küme adı.

    Bu komutlar bilgi aşağıdakine benzer döndürün:

        wn0-dest.aazwc2onlofevkbof0cuixrp5h.gx.internal.cloudapp.net:9092,wn1-dest.aazwc2onlofevkbof0cuixrp5h.gx.internal.cloudapp.net:9092

4. Oluşturmak için aşağıdakileri kullanın bir `producer.properties` ile iletişim kurmak açıklar dosya **hedef** küme:
...
2. SSH bağlantı **kaynak** küme, bir üretici başlatmak ve konu başlığına ileti göndermek için aşağıdaki komutu kullanın:
    ...
    Değiştir `$PASSWORD` kaynak kümesi için oturum açma (Yönetici) parolayla.

    Değiştir `$CLUSTERNAME` kaynak kümesi adı.

     Boş bir satırı bir imleç ile geldiğinde, birkaç metin iletileri yazın. Bu konu başlığına gönderilen **kaynak** küme. İşiniz bittiğinde, kullanın **Ctrl + C** üretici işlemi sonlandırmak için.

3. SSH bağlantı **hedef** küme, kullanın **Ctrl + C** MirrorMaker işlemi sonlandırmak için. Doğrulamak için aşağıdaki komutları kullanın `testtopic` konu oluşturuldu ve bu verileri konusunda bu yansıtmaya çoğaltılan:
    ...
    Değiştir `$PASSWORD` hedef küme için oturum açma (Yönetici) parolayla.
