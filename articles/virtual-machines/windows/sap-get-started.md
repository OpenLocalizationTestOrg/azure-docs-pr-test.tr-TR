...
---
# <a name="using-sap-on-azure-windows-virtual-machines-vms"></a>SAP Azure Windows sanal makinelerde (VM'ler) kullanma
...
Microsoft Azure, SAP hazır bulut iş ortağı olarak seçerek görev kritik SAP iş yükünü scaaleable, uyumlu ve işletme kanıtlanmış platform çalıştırmak mümkün tooreliably olacaktır.  Merhaba scaleability, flexability, alma ve Azure'nın tasarruf maliyeti. Microsoft ve SAP arasındaki genişletilmiş hello ortaklığı ile azure - geliştirme ve test ve üretim senaryolarında arasında SAP uygulamaları çalıştırabilir ve tam olarak desteklenmektedir. SAP NetWeaver tooSAP S4/HANA, Linux tooWindows, SAP HANA tooSQL biz, kapsamdaki vardır. 

Microsoft Azure sanal makine hizmetlerini ve Azure büyük örneklerinde SAP HANA ile Microsoft kapsamlı bir hizmet olarak altyapı (Iaas) platformu sunar. Azure üzerinde bir geniş SAP çözümleri desteklenmediğinden bu "alma başlatılan belge hizmet içindekiler tablosu geçerli bizim SAP belgeleri kümesi için. Daha fazla başlıkları tooour belge kitaplığı - eklendiklerinde bunları buraya eklenen görürsünüz. 

## <a name="sap-hana-certifications-on-microsoft-azure"></a>SAP HANA sertifikalarından Microsoft Azure
| SAP Ürünü | Desteklenen İşletim Sistemi | Azure Teklifleri |
| --- | --- | --- |
| SAP HANA Geliştirici sürümü (Merhaba HANA istemci yazılımı da dahil olmak üzere SQLODBC, yalnızca ODBO Windows, ODBC, JDBC sürücüsü HANA oluşan studio ve HANA veritabanına) |Red Hat Enterprise Linux, SUSE Linux Enterprise |A7, A8 |
| MHANA bir |Red Hat Enterprise Linux, SUSE Linux Enterprise |DS14_v2 (genel kullanıma sunulduktan sonra) |
| SAP S/4HANA |Red Hat Enterprise Linux, SUSE Linux Enterprise |Denetimli kullanılabilirlik için GS5, SAP HANA azure'da (büyük örnekleri) |
| Suite on HANA, OLTP |Red Hat Enterprise Linux, SUSE Linux Enterprise |Azure’da SAP HANA (Büyük örnekler) |
...
| SAP BW/4HANA |Red Hat Enterprise Linux, SUSE Linux Enterprise |SAP HANA (büyük örnekler) azure'da GS5 tek düğümlü dağıtımlar için |

## <a name="sap-netweaver-certifications"></a>SAP NetWeaver sertifikaları
Microsoft Azure SAP ürünleri, Microsoft ve SAP tam destek ile aşağıdaki hello için sertifikalıdır.

| SAP Ürünü | Konuk işletim sistemi | RDBMS | Sanal Makine Türleri |
| --- | --- | --- | --- |
| SAP Business Suite Yazılımı |Windows, SUSE Linux Enterprise, Red Hat Enterprise Linux |SQL Server, Oracle (yalnızca Windows), DB2, SAP ana |A5 tooA11 D11 tooD14, DS11 tooDS14, GS1 tooGS5 |
| SAP Business All-in-One |Windows, SUSE Linux Enterprise, Red Hat Enterprise Linux |SQL Server, Oracle (yalnızca Windows), DB2, SAP ana |A5 tooA11 D11 tooD14, DS11 tooDS14, GS1 tooGS5 |
| SAP BusinessObjects BI |Windows |Yok |A5 tooA11 D11 tooD14, DS11 tooDS14, GS1 tooGS5 |
| SAP NetWeaver |Windows, SUSE Linux Enterprise, Red Hat Enterprise Linux |SQL Server, Oracle (yalnızca Windows), DB2, SAP ana |A5 tooA11 D11 tooD14, DS11 tooDS14, GS1 tooGS5 |
...
Özet: Bu Hızlı Başlangıç Kılavuzu, bir tek örnekli SAP HANA prototip/tanıtım sistemi Azure vm'lerinde el ile yükleme tarafından SAP NetWeaver 7.5 ve SAP HANA SP12 tooset yardımcı olur. Merhaba Kılavuzu hello okuyucu toodeploy sanal makineleri veya sanal ağları da hello Azure portal veya Powershell/CLI dahil olmak üzere üzerinden seçeneği toouse json şablonları nasıl hello gibi Azure Iaas temel kavramları hakkında bilgi sahibi olduğunu varsayar. Ayrıca hello okuyucu SAP HANA, SAP NetWeaver ve nasıl tooinstall, şirket içi alışkın olduğu beklenir.

Güncelleştirilmiş: Eylül 2016

[Bu kılavuz burada bulunabilir.](../virtual-machines-linux-sap-hana-get-started.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json)
...
[Bu kılavuz burada bulunabilir.](../virtual-machines-linux-sap-on-suse-quickstart.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json)

## <a name="deploying-sap-ides-ehp7-sp3-for-sap-erp-60-on-microsoft-azure"></a>Microsoft Azure üzerinde SAP ERP 6.0 için SAP IDE EHP7 SP3 dağıtma
Başlık: Azure vm'lerinde SAP HANA el ile yüklenmesi için aaaQuickstart Kılavuzu

Özet: Bu nasıl toodeploy SAP makalede SQL Server ve Windows işletim sistemi ile Microsoft Azure üzerinde SAP bulut Gereci kitaplığı 3.0 üzerinden çalışan IDE. 

Güncelleştirilmiş: Eylül 2016

[Bu kılavuz burada bulunabilir.](sap-cal-ides-erp6-ehp7-sp3-sql.md?toc=%2fazure%2fvirtual-machines%2fwindows%2ftoc.json)

## <a name="3da0389e-708b-4e82-b2a2-e92f132df89c"></a>Planlama ve uygulama
Başlık: aaaSAP NetWeaver Azure Virtual Machines'de (VM'ler) – planlama ve Uygulama Kılavuzu

Özet: SAP NetWeaver Azure sanal makinelerde çalışan hakkında düşünüyorum, bu hello kağıt toostart sahip olur. Bu planlama ve Uygulama Kılavuzu, mevcut olup olmadığını değerlendirmek yardımcı olur veya planlı SAP NetWeaver tabanlı sistem dağıtılan tooan Azure sanal makineleri ortam olabilir. Birden çok SAP NetWeaver dağıtım senaryoları kapsar ve belirli tooAzure SAP yapılandırmaları içerir. Merhaba kağıt listeler ve ihtiyacınız vardır tüm hello gerekli yapılandırma hakkında bilgi hello SAP/Azure yan toorun karma SAP yatay açıklar. SAP NetWeaver tabanlı sistemler yüksek kullanılabilirliğini tooensure Iaas'da sürebilir ölçüleri de kapsar.

Güncelleştirilmiş: Ağustos 2016

[Bu kılavuz burada bulunabilir.][planning-guide]

## <a name="6aadadd2-76b5-46d8-8713-e8d63630e955"></a>Dağıtım
Başlık: aaaSAP NetWeaver Azure Virtual Machines'de (VM'ler) – dağıtım kılavuzu
...
Başlık: aaaSAP NetWeaver Azure Virtual Machines'de (VM'ler) – DBMS Dağıtım Kılavuzu

Özet: Bu yazıda SAP ile birlikte çalışması gerektiğini hello DBMS sistemleri için planlama ve uygulama konuları kapsar. Merhaba ilk bölümünde genel konular listelenen ve sunulan. Merhaba hello kağıt aşağıdaki bölümlerini SAP tarafından desteklenen farklı DBMS Azure toodeployments ilgilidir. Yer verilen DBMS bileşenleri SQL Server, SAP ASE ve Oracle sistemleridir. Bu belirli bölümlerinde, SAP sistemleri Azure üzerinde bu DBMS birlikte çalışmadığı zaman tooaccount sahip konuları ele alınmıştır. Konular Azure ile ilgili farklı DBMS SAP uygulamalarla hello kullanım için sunulan hello tarafından desteklenen yedekleme ve yüksek oranda kullanılabilir yöntemleri ister.

Güncelleştirilmiş: Ağustos 2016

[Bu kılavuz burada bulunabilir.][dbms-guide]

## <a name="63dab028-2c4f-4636-8f99-90bbb264eaba"></a>Yüksek kullanılabilirlik Dağıtım Kılavuzu
Başlık: aaaSAP NetWeaver Azure Virtual Machines'de (VM'ler) – yüksek kullanılabilirlik Dağıtım Kılavuzu

Özet: Bu belgede, Azure'da hello SAP ASCS/SCS ve DBMS gibi hata bileşenleri tek noktası SAP nasıl korunabilir açıklanmaktadır. Bileşenleri hello SAP ASCS/SCS, DBMS ve uygulama sunucuları SAP NetWeaver sistemleri, örn. SAP NetWeaver ABAP, SAP NetWeaver Java, SAP NetWeaver ABAP + Java sistemleri hello işlevleri için gereklidir. Bu nedenle, yüksek kullanılabilirlik işlevselliği gereksinimlerini toobe koyun toomake bu bileşenlerin tam ve Hyper-V ortamları için Windows Küme yapılandırmaları ile bitti olarak bir sunucu veya bir VM başarısızlığını karşılayabilir emin yerleştirin.

Güncelleştirilmiş: Aralık 2016

[Bu kılavuz burada bulunabilir.][ha-guide]
