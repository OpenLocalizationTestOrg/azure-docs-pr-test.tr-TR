...
Hello aynı birden çok sanal makine oluşturulan bulut hizmeti. toounderstand bu sanal makineleri senaryosu, bkz: [nasıl tooconnect sanal makineleri bir sanal ağ veya Bulut hizmeti ile](../classic/connect-vms.md#connect-vms-in-a-standalone-cloud-service). Bu senaryoda, tooconnect tooSQL Server çalışan bir sanal makine bir istemcide çalışır aynı bulut hizmeti hello başka bir sanal makinede.
...
Bu bağlantı dizesi bağlantısı üzerinden istemciler için sağlar, ancak internet Merhaba, bu herkes tooyour SQL Server bağlanabilir anlamına gelmez. Dış istemcileri toohello doğru kullanıcı adı ve parolaya sahip. Ek güvenlik için hello iyi bilinen bağlantı noktası 1433 hello genel sanal makine uç noktası için kullanmayın. Ve mümkünse, bir ACL izin uç nokta toorestrict trafiği yalnızca toohello istemcilerinize eklemeyi düşünün. ACL'ler uç ile kullanma ile ilgili yönergeler için bkz: [uç noktada Yönet hello ACL](../classic/setup-endpoints.md#manage-the-acl-on-an-endpoint).

> [!NOTE]
> Bu teknik toocommunicate SQL Server ile kullandığınızda, tüm giden hello Azure veri merkezi verilerdir konu toonormal [giden veri aktarımları fiyatlandırma](https://azure.microsoft.com/pricing/details/data-transfers/).
> 
> 
...
Sanal ağlar, toojoin de Azure Vm'lerde tooa etki alanınızın etkinleştirin. Tooa birleştirme hello tek yolu toouse Windows kimlik doğrulaması SQL Server ile etki alanıdır. Merhaba bağlantı gerektiren diğer senaryolar SQL kimlik doğrulaması kullanıcı adları ve parolalar.

Tooconfigure bir etki alanı ortamı ve Windows kimlik doğrulaması düşünüyorsanız, düzenleme tooconfigure hello ortak uç nokta veya hello SQL kimlik doğrulaması ve oturum açmalar gerekmez. Bu senaryoda, hello bağlantı dizesinde hello SQL Server VM adı belirterek tooyour SQL Server örneği bağlanabilir. Aşağıdaki örneğine hello Windows kimlik doğrulaması yapılandırılan ve o hello kullanıcıya erişim toohello SQL Server örneği verildi varsayar.
...
Toohello SQL Server'ın başka bir sanal makineden bağlanın ya da Internet hello önce hello aşağıdaki görevleri tamamlamanız gerekir:
...
Bir Azure sanal makinede çalışan SQL Server için en iyi güvenlik uygulamaları hello tüm önemli tooreview olur. Daha fazla bilgi için bkz. [Azure Sanal Makineler'de SQL Server için Güvenlikle İlgili Dikkat Edilmesi Gerekenler](../sql/virtual-machines-windows-sql-security.md).
