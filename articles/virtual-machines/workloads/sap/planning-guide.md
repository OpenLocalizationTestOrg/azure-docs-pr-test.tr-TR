...
> [!NOTE]
> Şirket içi dağıtımları SAP sistemleri çalışan Azure sanal makineleri bir şirket içi etki alanının üyesi olduğu SAP sistemlerinin üretim SAP sistemleri için desteklenir. Şirketler arası yapılandırmalar bölümleri dağıtmak için desteklenen veya Azure'da SAP Windows'un tamamlayın. Bile tam SAP yatay Azure'da çalışan, şirket içi etki alanı ve REKLAM/OpenLDAP parçası olan bu VM'lerin olması gerektirir. Belge önceki sürümlerinde, biz 'Karma' terimi şirket içi bağlantılar şirket içi ve Azure arasında olduğunu aslında burada kökü karma BT senaryoları hakkında açıklandı. Ayrıca, Azure sanal makineleri şirket içi Active Directory'nin parçası bulunmasına / OpenLDAP.
>
>
...
* Seçilebilen en düşük düzeydir 'eşdeğeri olan yerel artıklık', aynı veri merkezinde bir Azure bölgesinin içindeki verilerin üç çoğaltma (bölüm bkz [Azure bölgeleri][planning-guide-3.1]).
...
Azure Storage hesapları hakkında daha fazla bilgi bulunabilir [bu makalede][storage-scalability-targets]. Bu makaleyi okuduktan, sınırlamalar Azure standart depolama hesapları ve Premium depolama hesapları arasındaki farkları olduğunu unutmayın. Bu tür bir depolama hesabında depolanan veri hacmine önemli farklılıklar şunlardır. Standart depolama, Premium Storage ile büyük bir büyüklük birimdir. Diğer tarafta standart depolama hesabı ancak böyle bir kısıtlama Azure Premium depolama hesabı vardır ('Toplam istek oranı' sütununa bakın) IOP cinsinden ciddi bir şekilde sınırlıdır. Ayrıntıları ve bu farklılıklar sonuçlarını SAP sistemleri, özellikle DBMS sunucularda dağıtımları ele alırken aşağıdakiler ele alınacaktır.
...
> ![Windows][Logo_Windows] Windows
>
> VM bir yönetici hesabıyla oturum açmak için son adım olacaktır. 'Yönetici' olarak bir Windows komut penceresi açın. İçin %Windir%\Windows\System32\sysprep gidin ve sysprep.exe yürütün.
> Küçük bir pencere görüntülenir. (Varsayılan atanmamış checked) 'Generalize' seçeneği işaretleyin ve kapatma seçeneği 'Kapatmak için', 'Yeniden başlatma' varsayılandan değiştirmek önemlidir. Bu yordam, sysprep işleminin yürütülen şirket içi bir VM konuk işletim sistemini olduğunu varsayar.
...
* (İsteğe bağlı) VHD ile yönetilen bir Disk oluşturmak *az disketi* -https://docs.microsoft.com/cli/azure/disk#create bakın
...
* Aboneliğinizi seçin *az hesabı kümesi--abonelik`<subscription name or id`>*
* VHD ile karşıya *az depolama blob karşıya yükleme* -bkz [Azure Storage ile Azure CLI kullanma][storage-azure-cli]
* (İsteğe bağlı) VHD ile yönetilen bir Disk görüntüsü oluşturmak *az görüntü oluşturma* -https://docs.microsoft.com/cli/azure/image#create bakın
...
#### <a name="powershell"></a>PowerShell
  ...
  * Bir VHD indirme  
  SAP sistem durdurulur ve VM'yi kapatın sonra şirket içi world VHD diskleri indirmek için şirket içi hedef kaydetme AzureRmVhd PowerShell cmdlet'ini kullanabilirsiniz. İçinde bulabileceğiniz VHD URL'si, bunu yapabilmek 'Depolama bölümünü' Azure Portalı'nı (depolama hesabı ve VHD oluşturulduğu depolama kapsayıcısı gezinmek için gereklidir) ve VHD burada kopyalanacağı bilmeniz gerekir.
...
> Bunu yapmak için:
>
> * Açık denetim masası\sistem ve güvenlik\windows Güvenlik Duvarı'nda 'Gelişmiş Ayarlar'.
> * Şimdi gelen kuralları ve seçtiğiniz 'Yeni Kural' ni sağ tıklatın.
> * Yeni bir 'Bağlantı noktası' kuralı oluşturmak için aşağıdaki Sihirbazı'nı seçerseniz.
...
> * Sihirbazın sonraki adımına 'İşaretli bağlantı izin ver' öğesini bırakın gerekir.
...
> * Sihirbazın son adımda, kuralın adı ve 'Bitiş' tuşuna basarak kaydedin
>
...
> Bunu yapmak için:
>
> * Bazı ağ yazıcıları yazıcınızın Azure VM'deki ayarlamak kolay hale getiren bir Yapılandırma Sihirbazı ile gelir. Herhangi bir Sihirbazı yazılım yazıcıyla dağıtılıyorsa yazıcıyı ayarlamak için "manual" yeni bir TCP/IP yazıcı bağlantı noktasını oluşturmak için yoludur.
...
Ancak, yine müşteri tarafından yürütülmesi gereken bir adım yoktur. Bu etkinleştirme ve yapılandırma performans koleksiyonunun adıdır. 'Configuration' ilgili işlemi, bir PowerShell komut dosyası veya CLI komutu tarafından otomatik olarak. Bölümünde açıklandığı gibi Microsoft Azure Script Center PowerShell Betiği indirilebilir [Dağıtım Kılavuzu'na][deployment-guide].
...
#### <a name="utilizing-azure-infrastructure-vm-restart-to-achieve-higher-availability-of-sap-applications"></a>"Yüksek" SAP uygulamalarının kullanılabilirliğini elde etmek için kullanan Azure altyapı VM yeniden başlatma
