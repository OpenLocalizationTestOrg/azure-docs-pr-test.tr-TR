...
Ya da son değişiklik zamanı BLOB'lar hariç tutmak istediğiniz ise, aynı veya daha eski hedef dosya ekleme `/XO` seçeneği:
...
Unutmayın `/SyncCopy` zaman uyumsuz kopyaya karşılaştırma maliyet ek çıkış oluşturabilir, Azure VM'deki çıkışı maliyeti önlemek için kaynak depolama hesabınız ile aynı bölgede olan bu seçeneği kullanmak için önerilen yaklaşımdır.
...
### <a name="export-table-to-json-or-csv-data-file-format"></a>Tablo verme JSON veya CSV veri dosyası biçimi
...
### <a name="export-table-to-blob"></a>BLOB tabloyu dışarı aktarma
...
### <a name="import-entities-to-table-using-blobs"></a>BLOB'ları kullanarak tablo varlıkları İçeri Aktar
...
Özellik "AzureStorageUseV1MD5" • True - varsayılan değer, AzCopy .NET MD5 uygulama kullanır.
