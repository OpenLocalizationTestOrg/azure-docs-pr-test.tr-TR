...
-   Kaçının `single-transaction` çok büyük veritabanları çıkarırken mysqldump seçeneği. Tek bir işlem içinde çok sayıda tabloya dökme ek depolama alanı neden olur ve bellek kaynaklarının sırasında tüketilen geri yüklemek ve performans gecikmeler veya kaynak kısıtlamaları neden olabilir.
-   Veritabanları çıkarırken deyimi yürütme yükünü en aza indirmek için SQL ile yüklenirken kullanım çoklu değer ekler. Çoklu değer ekler mysqldump yardımcı programı tarafından üretilen döküm dosyaları kullanırken, varsayılan olarak etkinleştirilir. 
