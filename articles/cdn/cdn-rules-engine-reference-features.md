...
- Ad yalnızca değer seçeneğinde tanımlı olduğundan emin olun 
...
Devre dışı|Bant genişliği azaltma parametreleri yoksaymak kenar sunucularımızda neden olur. İstenen içerik (yani, bant genişliği azaltma olmadan) normal olarak sunulacak.
...
Bu özellik için örnek kullanım aşağıda verilmiştir. Örnek istek ve varsayılan önbellek anahtarını aşağıda verilmiştir.
...
--->
Kısmi önbellek isabetsizliği genellikle bir kullanıcı bir indirme durdurur sonra veya yalnızca HTTP Aralık isteklerini kullanarak istenen varlıklar için oluşur. Bu özellik, burada kullanıcılar değil genellikle yükleyecek onlardan büyük varlıklar baştan için en yararlı olur (örneğin, videoları). Sonuç olarak, bu özellik HTTP büyük platform üzerinde varsayılan olarak etkindir. Diğer tüm platformlarda devre dışı bırakılır.
...
Bir dosya biçimi, kendi Internet medya türü (yani, Content-Type) kullanılarak belirtilebilir. Internet medya türü belirli bir varlık dosya biçimi tanımlamak sunucularımızda sağlayan platformdan bağımsız meta verilerdir. Ortak Internet medya türleri listesi aşağıda verilmiştir.
...
- Bu özellik için bir kural eklemeden önce bu kuralın uygulanacağı platform için sıkıştırma sayfasında sıkıştırma devre dışı seçeneğini ayarladığınızdan emin olun.
...
- Bu özellik tarafından yapılandırın:
    ...
    - Bir tamsayı değeri belirtme ve istediğiniz zaman birimi (yani, saniye, dakika, saat, vb.) seçme. Bu değer varsayılan iç max-age aralığı tanımlar.
...
- Bu özellik bir tamsayı değeri belirterek ve istediğiniz zaman birimi (yani, saniye, dakika, saat, vb.) seçerek yapılandırın.
...
- Bu özellik tarafından yapılandırın:
    ...
    - Bir tamsayı değeri belirtme ve istediğiniz zaman birimi (yani, saniye, dakika, saat, vb.) seçme. Bu değer isteğin, max-age aralığı tanımlar.
...
- Bu özellik tarafından yapılandırın:
    ...
    - Bir tamsayı değeri belirtme ve istediğiniz zaman birimi (yani, saniye, dakika, saat, vb.) seçme. Bu değer iç max-uygulanacak olan eski tanımlar.
...
Değer|Sonuç
--|--
Etkin|Varlık kaynak sunucudan yeniden getirilemiyor bizim uç sunucusunu neden olur.
...
- Bu özellik davranışını seçilen zaman birimi göre değişir.
    - **Zaman birimi:** eski içerik teslim izin vermek için bir zaman birimi (örn., saniye, dakika, saat, vb.) seçin ve bir süre belirtin. Bu tür kurulum teslim edebilir süreyi uzatmak CDN doğrulama aşağıdaki formülü göre istemeden önce içerik verir:**TTL** + **eski sırada düzeltin zaman** 
...
- Yalnızca alfasayısal karakterler kullandığınızdan emin olun.
...
-|-|-
Ekle|Belirtilen değer var olan isteği üstbilgi değeri toend eklenir.|**Üstbilgi değeri (istemci) istek:**Value1 <br/> **Üstbilgi değeri (HTTP kurallar altyapısı) istek:** Value2 <br/>**Yeni istek üstbilgi değeri:** Value1Value2
...
- Servis talebi üstbilgi tanımlamak amacıyla dikkate alınmaz. Örneğin, aşağıdaki Cache-Control üstbilgisi adı varyasyonları hiçbirini tanımlamak için kullanılabilir:
    ...
    - CACHE-CONTROL
    - cachE-Control
- Bir üstbilgi adı belirtirken yalnızca alfasayısal karakterler, tire ve alt çizgi kullandığınızdan emin olun.
...
Seçenek|Açıklama|Örnek
-|-|-
Ekle|Belirtilen değer var olan isteği üstbilgi değeri toend eklenir.|**Yanıt üstbilgi değeri (istemci):**Value1 <br/> **Yanıt üstbilgi değeri (HTTP kurallar altyapısı):** Value2 <br/>**Yeni yanıt üstbilgi değeri:** Value1Value2
...
Belirtilen üstbilgi adı aşağıdakilerden herhangi birini eşleşmediğini emin olun:
...
- Bu alanın geçerli karakterler şunlardır: alfasayısal (yani, 0-9, a-z ve A-Z), tire, iki nokta üst üste, noktalı, kesme, virgül, nokta, alt çizgi, eşittir işareti, parantez, köşeli ayraçlar ve boşluk. Süslü ayraçlar ve yüzde simge yalnızca bir üstbilgi alanı belirtmek için kullanılan olduğunda izin verilir.
