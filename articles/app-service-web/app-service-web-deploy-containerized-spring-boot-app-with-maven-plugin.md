...
---
# <a name="how-toouse-hello-maven-plugin-for-azure-web-apps-toodeploy-a-containerized-spring-boot-app-tooazure"></a>Nasıl toouse hello Maven eklentisi için Azure Web Apps toodeploy kapsayıcılı yay önyükleme uygulama tooAzure
...
Bu makalede Azure Web Apps toodeploy hello Maven eklentisi kullanarak bir örnek yay önyükleme uygulama Docker kapsayıcısı tooAzure uygulama hizmetleri gösterilmektedir.
...
Bu bölümde, Maven kapsayıcı tooAzure dağıtırken kullanacağı, Azure hizmet asıl tooconfigure hello kimlik doğrulamasını hello değerleri kullanın.
...
## <a name="optional-deploy-your-local-docker-file-toodocker-hub"></a>İsteğe bağlı: yerel, Docker dosya tooDocker hub'ı dağıtma

Docker hesabınız varsa, Docker kapsayıcısı görüntüsünü yerel olarak oluşturabilirsiniz ve tooDocker Hub gönderme. toodo, bu nedenle, hello aşağıdaki adımları kullanın.

1. Açık hello `pom.xml` dosyasını bir metin düzenleyicisinde yay önyükleme uygulamanız için.

1. Merhaba bulun `<imageName>` hello alt öğesi `<containerSettings>` öğesi.

1. Güncelleştirme hello `${docker.image.prefix}` Docker hesap adınızı değerle:
   ```xml
   <containerSettings>
...
1. Dağıtım yöntemleri aşağıdaki hello birini seçin:

   * Kapsayıcı görüntünüzü Maven ile yerel olarak oluşturun ve ardından Docker toopush, kapsayıcı tooDocker Hub kullanın:
      ```shell
      mvn clean package docker:build
      docker push
      ```
   
   * Merhaba varsa [Maven için Docker eklentisi] yüklü, otomatik olarak oluşturabilir ve kullanarak, kapsayıcı görüntü tooDocker Hub hello `-DpushImage` parametre:
      ```shell
      mvn clean package docker:build -DpushImage
      ```

## <a name="optional-customize-your-pomxml-before-deploying-your-container-tooazure"></a>İsteğe bağlı:, pom.xml kapsayıcı tooAzure dağıtmadan önce özelleştirme
...
`<resourceGroup>` | Olan hello hedef kaynak grubu belirtir `maven-plugin` Bu örnekte. zaten yoksa, dağıtım sırasında hello kaynak grubu oluşturulur.
...
## <a name="build-and-deploy-your-container-tooazure"></a>Derleme ve kapsayıcı tooAzure dağıtma

Bölümler, bu makalenin önceki hello tüm hello ayarlarını yapılandırdıktan sonra hazır toodeploy olan kapsayıcı tooAzure. toodo, bu nedenle, hello aşağıdaki adımları kullanın:
...
* [Nasıl toouse hello Maven eklentisi için Azure Web Apps toodeploy yay önyükleme uygulama tooAzure uygulama hizmeti](app-service-web-deploy-spring-boot-app-with-maven-plugin.md)
