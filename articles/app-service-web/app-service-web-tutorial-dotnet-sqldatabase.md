...
[Azure Web Apps](https://docs.microsoft.com/azure/app-service-web/app-service-web-overview) yüksek oranda ölçeklenebilen, kendi kendine düzeltme eki uygulayan bir web barındırma hizmeti sunar. Bu öğreticide, bir veri tabanlı ASP.NET web uygulamasını Azure dağıtmak ve buna bağlanmak nasıl gösterilir [Azure SQL veritabanı](../sql-database/sql-database-technical-overview.md). İşiniz bittiğinde, çalışır durumda bir ASP.NET uygulamanız [Azure App Service](../app-service/app-service-value-prop-what-is.md) ve SQL veritabanına bağlı.
...
Seçin `DotNetAppSqlDb` SQL veritabanı. Daha önce oluşturduğunuz bağlantı altındaki otomatik olarak doldurulur.
...
Sağ `Todoes` tablo ve seçin **görünüm verilerini**. 
...
Bul `<td>` içeren öğeyi `Html.ActionLink()` yardımcı yöntemler. Bu öğe yalnızca aşağıdaki Razor kodu ekleyin:
...
Tarayıcınızda, yapılacaklar listesi uygulaması Azure geçici tıklatmayı deneyin. İzleme iletileri şimdi akışı **çıkış** Visual Studio'daki.
