...
Uygulamanızı her modül için TelemetryClient örneğini kullanmanızı öneririz. Örneğin, gelen HTTP isteklerini ve başka bir ara yazılım sınıfında rapor iş mantığı olaylarını bildirmek için web hizmetiniz bir TelemetryClient örneği olabilir. Özellikleri gibi ayarlayabilirsiniz `TelemetryClient.Context.User.Id` kullanıcılar ve oturumları izlemek için veya `TelemetryClient.Context.Device.Id` makine tanımlamak için. Bu bilgiler örneği gönderdiği tüm olayları eklenir.
