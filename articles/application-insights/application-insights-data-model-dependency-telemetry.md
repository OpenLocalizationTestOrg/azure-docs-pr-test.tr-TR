...
Bağımlılık Telemetrisi (içinde [Application Insights](app-insights-overview.md)) hello izlenen bileşenin SQL ya da bir HTTP uç noktası gibi uzak bir bileşeni ile etkileşim temsil eder.

## <a name="name"></a>Ad

Bu bağımlılık çağrısı ile başlatılan hello komut adı. Düşük önem düzeyi değeri. Saklı yordam adı ve URL yolu şablonunu örnektir.

## <a name="id"></a>Kimlik

Bir bağımlılık çağrısı örneği tanımlayıcısı. Bağıntı için hello istek telemetri öğesi toothis bağımlılık çağrısı karşılık gelen ile kullanılır. Daha fazla bilgi için bkz: [bağıntı](application-insights-correlation.md) sayfası.
