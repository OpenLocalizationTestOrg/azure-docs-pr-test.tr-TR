...
Filtreleri, bir şekilde tooselect hello telemetri sağlar, [Java web uygulaması gönderir tooApplication Öngörüler](app-insights-java-get-started.md). Kullanabileceğiniz bazı Giden kutusu filtreler vardır ve kendi özel filtreler de yazabilirsiniz.

Merhaba Giden kutusu filtreler aşağıdakileri içerir:

* İzleme önem düzeyi
* Belirli URL'leri, anahtar sözcükleri veya yanıt kodları
* Hızlı yanıtlar - diğer bir deyişle, istekleri toowhich uygulamanızı yanıt tooquickly
* Belirli olay adları

> [!NOTE]
> Filtreler, uygulamanızın hello ölçümleri eğme. Örneğin, sipariş toodiagnose yavaş yanıtları, bir filtre toodiscard hızlı yanıt süreleri ayarlayın, karar verebilirsiniz. Ancak Application Insights tarafından bildirilen hello ortalama yanıt sürelerini sonra hello true hızından daha yavaş olacaktır ve isteği hello sayısı hello gerçek sayısından küçük olmalıdır.
...
[Yerleşik işlemciler Hello kümesini İnceleme](https://github.com/Microsoft/ApplicationInsights-Java/tree/master/core/src/main/java/com/microsoft/applicationinsights/internal/processor).
...
* `DurationThresholdInMS`-Süre tooload hello sayfa geçen toohello süre anlamına gelmektedir. Bu ayarlanırsa, bu süre daha hızlı yüklenen sayfaları raporlanmaz.
* `NotNeededNames`-Sayfa adları virgülle ayrılmış listesi.
* `NotNeededUrls`-URL virgülle ayrılmış listesini parça. Örneğin, `"home"` "home" Merhaba URL'de bulunan tüm sayfaları çıkışı filtreler.
...
Merhaba SyntheticSource özellik değerlerine sahip tüm telemetri çıkışı filtreler. Bunlar bot, örümcekler ve kullanılabilirlik testleri gelen istekleri içerir.
 ...
 *  İzleme - filtre yok. tooTrace düzeyine eşit
...
### <a name="2-invoke-your-filter-in-hello-configuration-file"></a>2. Filtreniz hello yapılandırma dosyasında çağırma
...
* Geçerli parametre değerleri sağladığınız denetleyin. Örneğin, süreleri tamsayı olmalıdır. Geçersiz değerler göz ardı hello filtre toobe neden olur. Özel filtre Oluşturucusu veya set yöntemi bir özel durum oluşturursa, göz ardı edilir.
