...
> [!IMPORTANT]
> Azure kaynaklarıyla çalışmadan önce Azure'da şu anda iki dağıtım modeli olduğunu önemli toounderstand olduğu: Azure Resource Manager ve klasik. Azure kaynaklarıyla çalışmadan önce [dağıtım modellerini ve araçlarlarını](../azure-resource-manager/resource-manager-deployment-model.md) iyice anladığınızdan emin olun. Bu makalenin hello üstünde hello sekmeleri tıklayarak farklı araçlarla ilgili hello belgeleri görüntüleyebilirsiniz. Bu makalede, hello Resource Manager dağıtım modeli yer almaktadır. 

[!INCLUDE [virtual-network-create-udr-scenario-include.md](../../includes/virtual-network-create-udr-scenario-include.md)]

## <a name="udr-resources-in-a-template-file"></a>Bir şablon dosyası UDR kaynakları
Görüntüleme ve hello karşıdan yükleme [örnek şablonu](https://github.com/telmosampaio/azure-templates/tree/master/IaaS-NSG-UDR).

Merhaba aşağıdaki bölümde gösterilmektedir hello hello tanımını Merhaba, ön uç UDR **azuredeploy-vnet-nsg-udr.json** hello senaryosu için dosyası:
...
tooassociate hello UDR toohello ön uç alt toochange hello alt ağı tanımını hello şablonda ve kullanım hello başvuru kimliği hello UDR için sahip.
...
Tooensure bu hello etmeniz **FW1** VM hello kullanılan tooreceive ve paketlerini iletmek NIC üzerinde etkin özelliği iletme hello IP sahiptir. gösterir hello tanımını aşağıdaki Hello bölümüne yukarıdaki hello senaryoyu temel FW1 hello azuredeploy nsg udr.json dosyasında için NIC hello.
...
2. Komut toocreate aşağıdaki hello bir kaynak grubu çalıştırın:

    ```powershell
    New-AzureRmResourceGroup -Name TestRG -Location westus
    ```

3. Komut toodeploy hello şablon aşağıdaki hello çalıştırın:
...
toodeploy hello ARM Şablonu'hello Azure CLI, tam hello aşağıdaki adımları kullanarak:
...
2. Komut tooswitch tooResource Yöneticisi modu aşağıdaki hello çalıştırın:
...
3. Tarayıcınızdan çok gidin**https://raw.githubusercontent.com/telmosampaio/azure-templates/master/IaaS-NSG-UDR/azuredeploy.parameters.json**hello hello json dosyasının içeriğini kopyalayın ve yeni bir dosyaya yapıştırın, bilgisayar. Bu senaryo için adlı tooa dosya altındaki hello değerler kopyaladığınız **c:\udr\azuredeploy.parameters.json**.
...
4. Toodeploy, yukarıda indirdiğiniz ve değiştirdiğiniz hello şablonu ve parametre dosyalarını kullanarak yeni Vnet'i hello komutu aşağıdaki hello çalıştırın:
...
5. Merhaba yeni kaynak grubunda oluşturduğunuz komut tooview hello kaynakları aşağıdaki hello çalıştırın:
...
> [!TIP]
> Tüm hello kaynakları görmüyorsanız hello çalıştırmak `azure group deployment show` komutu tooensure hello sağlama hello dağıtım durumu olan *Succeded*.
> 
