...
Bu konuda, bir Azure Resource Manager şablonunda kullanabileceğiniz tüm hello işlevler açıklanmaktadır.

Köşeli ayraç içinde yazarak, şablonlarda işlevler eklemek: `[` ve `]`sırasıyla. Merhaba ifade dağıtımı sırasında değerlendirilir. Bir dize yazılmış olsa da, hello ifade değerlendirme hello sonucunu bir dizi, nesne veya tamsayı gibi farklı bir JSON türde olabilir. JavaScript'te, işlev çağrıları olarak biçimlendirilmiş gibi yalnızca `functionName(arg1,arg2,arg3)`. Özellikler hello nokta ve [dizin] işleçleri kullanarak başvuru.

Bir şablon ifadesi 24.576 karakterden uzun olamaz.

Şablon işlevleri ve bunların parametrelerini büyük/küçük harf duyarlıdır. Örneğin, Resource Manager çözümler **variables('var1')** ve **VARIABLES('VAR1')** aynı hello gibi. Değerlendirildiğinde, hello işlevi açıkça durumda (örneğin, toUpper veya toLower) değiştirir sürece hello işlevi hello çalışması korur. Belirli kaynak türlerine işlevleri nasıl değerlendirildiği yedeklemiş servis talebi gereksinimleri olabilir.
...
Resource Manager işlevleri mantıksal koşulları ile çalışmak için aşağıdaki hello sunar:
...
* birden fazla şablon toomerge bakın [Azure Resource Manager ile bağlı şablonları kullanma](resource-group-linked-templates.md)
...
* toodeploy hello şablonu oluşturduğunuz toosee bkz [Azure Resource Manager şablonu ile bir uygulamayı dağıtma](resource-group-template-deploy.md)
