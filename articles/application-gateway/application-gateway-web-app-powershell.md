...
Uygulama ağ geçidi arka uç havuzu üye olarak toohave Azure Web uygulaması veya diğer çok kiracılı bir hizmet sağlar. Bu makalede, tooyou tooconfigure bir Azure web uygulaması uygulama ağ geçidi ile bilgi edinin. Merhaba ilk örneği gösterilir nasıl tooconfigure var olan bir uygulama ağ geçidi toouse arka uç havuzu üyesi olarak bir web uygulaması. Merhaba ikinci örneği gösterilir nasıl toocreate yeni bir uygulama ağ geçidi arka uç havuzu üyesi olarak bir web uygulaması ile.
...
Merhaba aşağıdaki örnek bir web uygulaması bir arka uç havuzu üye tooan varolan uygulama ağ geçidi olarak ekler. Her ikisi de anahtar hello `-PickHostNamefromBackendHttpSettings`hello araştırma yapılandırması ve `-PickHostNameFromBackendAddress` hello arka uç http ayarları web apps toowork sırada sağlanmalıdır.
...
Bu senaryo, bir web uygulaması başlatılan Web sitesi ve bir uygulama ağ geçidi alma hello asp.net ile dağıtır.
...
Bilgi nasıl tooconfigure yeniden yönlendirme adresi ziyaret ederek: [PowerShell ile uygulama ağ geçidi üzerinde yeniden yönlendirmeyi yapılandırma](application-gateway-configure-redirect-powershell.md).
