...
* Sanal makineler veya durumlarda hiçbiri [VM ölçek kümesi sağlıklı](#unhealthy-instances-in-backendaddresspool).
* Arka uç VM'ler veya VM ölçek kümesi örneklerinin [varsayılan durumu araştırması için yanıt vermiyor](#problems-with-default-health-probe.md).
...
Uygulama ağ geçidi hiçbir sanal makineleri veya arka uç adres havuzunda yapılandırılmış VM ölçek kümesi varsa, herhangi bir müşteri istek yönlendiremezsiniz ve hatalı ağ geçidi hata oluşturur.
...
Uygulama ağ geçidi için farklı havuzlar uygulanabilir BackendHttpSetting aracılığıyla bu ayarı yapılandırmak kullanıcıların sağlar. Farklı arka uç havuzları farklı BackendHttpSetting ve bu nedenle farklı istek zaman aşımına yapılandırılmış olabilir.
