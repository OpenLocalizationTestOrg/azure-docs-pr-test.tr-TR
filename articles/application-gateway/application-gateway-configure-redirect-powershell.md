...
---
# <a name="configure-redirection-on-application-gateway-with-powershell"></a><span data-ttu-id="40abf-103">PowerShell ile Application Gateway için yeniden yönlendirmeyi yapılandırma</span><span class="sxs-lookup"><span data-stu-id="40abf-103">Configure redirection on Application Gateway with PowerShell</span></span>

<span data-ttu-id="40abf-104">Uygulama ağ geçidi, tanımlı yapılandırmaya dayanarak trafiği yeniden yönlendirme özelliğini destekler.</span><span class="sxs-lookup"><span data-stu-id="40abf-104">Application gateway supports the ability to redirect traffic based on a defined configuration.</span></span> <span data-ttu-id="40abf-105">Genel olarak yeniden yönlendirme hakkında daha fazla bilgi edinmek için [Application Gateway yeniden yönlendirmeye genel bakış](application-gateway-redirect-overview.md) sayfasını ziyaret edin.</span><span class="sxs-lookup"><span data-stu-id="40abf-105">To learn more about redirection in general visit, [Application Gateway redirect overview](application-gateway-redirect-overview.md).</span></span> <span data-ttu-id="40abf-106">Bu makale HTTP’den HTTPS’e yeniden yönlendirme, yol tabanlı yeniden yönlendirme, çok siteli yeniden yönlendirme ve dış sitelere yeniden yönlendirme örnekleri içerir.</span><span class="sxs-lookup"><span data-stu-id="40abf-106">This article provides examples of HTTP to HTTPS redirection, path based redirects, multi-site redirects, and redirects to external sites.</span></span>

## <a name="http-to-https-redirect-on-an-existing-application-gateway"></a><span data-ttu-id="40abf-107">Mevcut uygulama ağ geçidinde HTTP’den HTTPS’e yeniden yönlendirme</span><span class="sxs-lookup"><span data-stu-id="40abf-107">HTTP to HTTPS redirect on an existing application gateway</span></span>

<span data-ttu-id="40abf-108">Aşağıdaki örnek, bağlantı noktası 443’te bulunan bir dinleyiciye yeniden yönlendirme isteği için bağlantı noktası 80’de yeni bir HTTP dinleyicisi oluşturur.</span><span class="sxs-lookup"><span data-stu-id="40abf-108">The following example creates a new HTTP listener on port 80 to redirect requests to the existing listener on port 443.</span></span>
...
<span data-ttu-id="40abf-109">Uygulama ağ geçidi güncelleştirildikten sonra bağlantı noktası 80’deki yeni dinleyiciyi test edin.</span><span class="sxs-lookup"><span data-stu-id="40abf-109">Once the application gateway is updated, test the new listener on port 80.</span></span>  <span data-ttu-id="40abf-110">Aşağıdaki örnekte, yeniden yönlendirmeyi test etmek için `curl` kullanılması gösterilir.</span><span class="sxs-lookup"><span data-stu-id="40abf-110">The following example shows using `curl` test the redirection.</span></span>
...
## <a name="path-based-redirect"></a><span data-ttu-id="40abf-111">Yol tabanlı yeniden yönlendirme</span><span class="sxs-lookup"><span data-stu-id="40abf-111">Path based redirect</span></span>

<span data-ttu-id="40abf-112">Aşağıdaki örnek, mevcut uygulama ağ geçidinde, yalnızca belirli bir url yoluna yeniden yönlendiren yeni bir yol tabanlı kural oluşturur.</span><span class="sxs-lookup"><span data-stu-id="40abf-112">The following example creates a new listener and a path based rule on an existing application gateway that redirects only traffic meeting the specific url path.</span></span>
...
## <a name="multi-site-redirect"></a><span data-ttu-id="40abf-113">Çok siteli yeniden yönlendirme</span><span class="sxs-lookup"><span data-stu-id="40abf-113">Multi-site redirect</span></span>

<span data-ttu-id="40abf-114">Aşağıdaki örnek, ağ geçidi 80’de 2 çok siteli dinleyiciyle yeni bir uygulama ağ geçidi oluşturur.</span><span class="sxs-lookup"><span data-stu-id="40abf-114">The following example creates a new application gateway with 2 multi-site listeners on port 80.</span></span> <span data-ttu-id="40abf-115">Dinleyiciler adatum.com ve adatum.org içindir.</span><span class="sxs-lookup"><span data-stu-id="40abf-115">The listeners are for adatum.com and adatum.org.</span></span> <span data-ttu-id="40abf-116">Adatum.org’dan adatum.com’a trafiği yeniden yönlendirmek için bir yeniden yönlendirme kuralı oluşturulur.</span><span class="sxs-lookup"><span data-stu-id="40abf-116">A redirect rule is created to redirect traffic from adatum.org to adatum.com.</span></span> <span data-ttu-id="40abf-117">CNAME diğer adlarını uygulama ağ geçidi genel IP adresine yapılandırmak için ek yapılandırmalar gerekir; etki alanınızı Azure DNS’ye devretme ve etki alanınız için CNAME kaydı oluşturma hakkında daha fazla bilgi için [Etki alanını Azure DNS’ye devretme](../dns/dns-delegate-domain-azure-dns.md) bölümünü ziyaret edin.</span><span class="sxs-lookup"><span data-stu-id="40abf-117">Additional configuration is required for configuring CNAME aliases to the application gateway public IP address, for more information on delegating your domain to Azure DNS and creating CNAME records for your domain visit, [Delegate a domain to Azure DNS](../dns/dns-delegate-domain-azure-dns.md).</span></span>
...
## <a name="redirect-to-an-external-site"></a><span data-ttu-id="40abf-118">Dış siteye yeniden yönlendirme</span><span class="sxs-lookup"><span data-stu-id="40abf-118">Redirect to an external site</span></span>

<span data-ttu-id="40abf-119">Aşağıdaki örnek, uygulama ağ geçidine gelen trafiği bir dış web sitesine (bing.com) yönlendirir.</span><span class="sxs-lookup"><span data-stu-id="40abf-119">The following example redirects traffic coming into the application gateway to an external website (bing.com).</span></span> <span data-ttu-id="40abf-120">`TargetUrl` anahtarını kullanırken, `IncludePath` ve `IncludeQuery` anahtarlarına izin verilmez.</span><span class="sxs-lookup"><span data-stu-id="40abf-120">When using the `TargetUrl` switch the `IncludePath` and `IncludeQuery` switches are not allowed.</span></span>
...
## <a name="next-steps"></a><span data-ttu-id="40abf-121">Sonraki adımlar</span><span class="sxs-lookup"><span data-stu-id="40abf-121">Next steps</span></span>

<span data-ttu-id="40abf-122">[Application Gateway’de PowerShell kullanarak uçtan uca SSL’yi yapılandırma](application-gateway-end-to-end-ssl-powershell.md) bölümünü ziyaret edin</span><span class="sxs-lookup"><span data-stu-id="40abf-122">Visit [configure end to end SSL with application gateway using PowerShell](application-gateway-end-to-end-ssl-powershell.md)</span></span>