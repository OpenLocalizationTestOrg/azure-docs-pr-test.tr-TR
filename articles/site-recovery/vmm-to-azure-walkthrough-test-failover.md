...
---
# <a name="step-11-run-a-test-failover-for-hyper-v-replication-with-vmm-tooazure"></a>11. adım: bir yük devretme sınaması için Hyper-V çoğaltma (VMM ile) tooAzure çalıştırma

Çalıştırdıktan sonra [Hyper-V sanal makineleri için çoğaltmayı etkinleştirme](vmm-to-azure-walkthrough-enable-replication.md), yük devretme testi şirket içi Hyper-V sanal makinelerden yönetilen System Center Virtual Machine Manager (VMM) Bulutlar tooAzure hello kullanarak bu makale toorun kullanmak [ Azure Site Recovery](site-recovery-overview.md) hello Azure portal hizmeti.
...
- Bu ayar etkinse, yalnızca kullanılabilirlik sahip kaynak gruplarını ayarlar **yönetilen diskleri kullanımını** etkin seçilebilir. Yönetilen bir diske sahip sanal makineleri kullanılabilirlik kümeleri ile birlikte olmalıdır **yönetilen diskleri kullanımını** çok ayarlamak**Evet**. Merhaba ayarı VM'ler için etkin değil, yalnızca kaynak gruplarındaki kullanılabilirlik kümelerini yönetilen diski etkin olmayan seçilebilir. [Daha fazla bilgi edinin](../virtual-machines/windows/manage-availability.md#use-managed-disks-for-vms-in-an-availability-set).
- - Çoğaltma için kullandığınız hello depolama hesabı depolama hizmeti şifrelemesi ile şifrelenmiş yönetilen diskleri yük devretme sırasında oluşturulamıyor. Bu durumda olmayan yönetilen diskleri kullanımını etkinleştirmek veya hello VM korumasını devre dışı bırakmak hem toouse şifreleme etkin olmayan bir depolama hesabı yeniden etkinleştirmek. [Daha fazla bilgi edinin](../virtual-machines/windows/managed-disks-overview.md#managed-disks-and-encryption).
...
- [Yeniden çalışma hakkında okuyun](site-recovery-failback-from-azure-to-hyper-v.md), toofail arka ve çoğaltma Azure Vm'leri yedekleme toohello birincil şirket içi VMM bulut.
