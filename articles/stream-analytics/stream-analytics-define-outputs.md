...
<td>Belirtilen kapsayıcı içinde bloblarınızın yazmak için kullanılan dosya yolu.<BR>Yol içinde BLOB'ları yazılır sıklığını belirtmek için şu 2 değişkenin bir veya daha fazla örneğini kullanmayı seçebilirsiniz:<BR>{date} {time}<BR>Örnek 1: cluster1/logs / {date} / {time}<BR>Örnek 2: cluster1/logs / {date}</td>
</tr>
<tr>
