...
## <a name="sql-server-dmvs-applicable-toosql-data-warehouse"></a>SQL Server Dmv'leri geçerli tooSQL veri ambarı
Aşağıdaki Dmv'leri geçerli tooSQL veri ambarı, ancak bunlar toohello bağlanarak yürütülmelidir hello **ana** veritabanı.
...
SQL veri ambarı birçok hello SQL Server dinamik yönetim görünümlerini (Dmv'leri) kullanıma sunar. Bu görünümler, SQL veri ambarı'nda sorgulandığında hello dağıtımları üzerinde çalışan SQL veritabanları hello durumu raporlama yapmayan.

SQL veri ambarı Microsoft'un MPP teknolojisi üzerine kurulu olduğundan, SQL veri ambarını ve çözümleme Platform Sistemi'ne'nın Paralel veri ambarı (PDW) kullanım aynı sistem görünümleri hello.

Her bu DMV's pdw_node_id olarak adlandırılan belirli bir sütuna sahip nedeni budur. Bu hello hello hello işlem düğümü tanımlayıcısıdır. PDW içinde hello işlem düğümü hello mimarisi için daha güçlü bir kavramdır. SQL veri ambarı'nda hello mimarisi hello dağıtımları hakkında daha fazla kullanır.

> [!NOTE]
> toouse bu görüntülemek, aşağıdaki tablonun hello gösterildiği gibi 'pdw_nodes_' hello ad alanının içine yerleştirin.
> 
> 

| SQL veri ambarı DMV adı | Bağlantı tooSQL Server Transact-SQL konu MSDN'de |
|:--- |:--- |
