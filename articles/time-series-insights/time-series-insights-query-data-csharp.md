...
---
# <a name="query-data-from-hello-azure-time-series-insights-environment-using-c"></a><span data-ttu-id="dac9a-103">C# kullanarak hello Azure zaman serisi Öngörüler ortamı verileri Sorgulama</span><span class="sxs-lookup"><span data-stu-id="dac9a-103">Query data from hello Azure Time Series Insights environment using C#</span></span>

<span data-ttu-id="dac9a-104">C# örnek nasıl tooquery verileri Azure zaman serisi Öngörüler ortamı hello gösterir.</span><span class="sxs-lookup"><span data-stu-id="dac9a-104">This C# example demonstrates how tooquery data from hello Azure Time Series Insights environment.</span></span>
<span data-ttu-id="dac9a-105">Merhaba örnek sorgu API kullanımı birkaç temel örnekler gösterilmektedir:</span><span class="sxs-lookup"><span data-stu-id="dac9a-105">hello sample shows several basic examples of Query API usage:</span></span>
1. <span data-ttu-id="dac9a-106">Hazırlık adımı olarak hello Azure Active Directory API'si aracılığıyla hello erişim belirteci alın.</span><span class="sxs-lookup"><span data-stu-id="dac9a-106">As a preparation step, acquire hello access token through hello Azure Active Directory API.</span></span> <span data-ttu-id="dac9a-107">Bunu hello belirteci geçirmek `Authorization` her sorgu API isteği üstbilgisi.</span><span class="sxs-lookup"><span data-stu-id="dac9a-107">Pass this token in hello `Authorization` header of every Query API request.</span></span> <span data-ttu-id="dac9a-108">Etkileşimli olmayan uygulamalar için bkz: [kimlik doğrulama ve yetkilendirme](time-series-insights-authentication-and-authorization.md).</span><span class="sxs-lookup"><span data-stu-id="dac9a-108">For setting up non-interactive applications, see [Authentication and authorization](time-series-insights-authentication-and-authorization.md).</span></span> <span data-ttu-id="dac9a-109">Ayrıca, hello hello örnek başında tanımlanan tüm hello sabitleri doğru ayarlandığından emin olun.</span><span class="sxs-lookup"><span data-stu-id="dac9a-109">Also, ensure all hello constants defined at hello beginning of hello sample are correctly set.</span></span>
2. <span data-ttu-id="dac9a-110">Kullanıcı hello ortamları Hello listesine erişim toois elde sahiptir.</span><span class="sxs-lookup"><span data-stu-id="dac9a-110">hello list of environments that hello user has access toois obtained.</span></span> <span data-ttu-id="dac9a-111">Merhaba ortamları birini ilgi hello ortamı olarak kayıt ve verileri bu ortam için daha fazla sorgulanır.</span><span class="sxs-lookup"><span data-stu-id="dac9a-111">One of hello environments is picked up as hello environment of interest, and further data is queried for this environment.</span></span>
3. <span data-ttu-id="dac9a-112">Bir HTTPS isteği örnek olarak, ilgi hello ortamı için kullanılabilirlik verileri istendi.</span><span class="sxs-lookup"><span data-stu-id="dac9a-112">As an example of HTTPS request, availability data is requested for hello environment of interest.</span></span>
4. <span data-ttu-id="dac9a-113">Web yuvası isteğini örnek olarak, olay verileri toplar ilgi hello ortamı için istenir.</span><span class="sxs-lookup"><span data-stu-id="dac9a-113">As an example of web socket request, event aggregates data is requested for hello environment of interest.</span></span> <span data-ttu-id="dac9a-114">Merhaba tüm kullanılabilirlik zaman aralığı için veri istendi.</span><span class="sxs-lookup"><span data-stu-id="dac9a-114">Data is requested for hello whole availability time range.</span></span>

## <a name="c-example"></a><span data-ttu-id="dac9a-115">C# örnek</span><span class="sxs-lookup"><span data-stu-id="dac9a-115">C# example</span></span>
...
## <a name="next-steps"></a><span data-ttu-id="dac9a-116">Sonraki adımlar</span><span class="sxs-lookup"><span data-stu-id="dac9a-116">Next steps</span></span>

<span data-ttu-id="dac9a-117">Merhaba Hello tam sorgu API başvuru için bkz: [sorgu API](/rest/api/time-series-insights/time-series-insights-reference-queryapi) belge.</span><span class="sxs-lookup"><span data-stu-id="dac9a-117">For hello full Query API reference, see hello [Query API](/rest/api/time-series-insights/time-series-insights-reference-queryapi) document.</span></span>
