...
Bir Data Lake Store hesabına erişimi olan bir Hdınsight kümesi oluşturduktan sonra verileri kopyalamak için Distcp'yi gibi Hadoop ekosistemi araçları kullanabilirsiniz **ve ondan** Data Lake Store hesabında bir Hdınsight küme depolama (WASB). Bu makalede, bunu elde etme hakkında yönergeler sağlar.
...
Hdınsight kümesi bir Hdınsight kümesine farklı kaynaklardan veri kopyalamak için kullanılan Distcp'yi yardımcı programı ile birlikte gelir. Hdınsight kümesi Data Lake Store ek depolama alanı kullanmak için yapılandırdıysanız, kullanılan out-of-- ve da Data Lake Store hesabından veri kopyalamak için box Distcp'yi yardımcı olabilir. Bu bölümde Distcp'yi yardımcı programını kullanmak üzere nasıl tümleştirildiği incelenmektedir.
...
2. Azure Storage Blobları (WASB) erişim olup olmadığını doğrulayın. Şu komutu çalıştırın:
    ...
    Bu depolama blobu içeriğini listesini sağlamanız gerekir.
...
4. Verileri WASB bir Data Lake Store hesabına kopyalamak için Distcp'yi kullanın.
    ...
    Bu içeriği kopyalayacak **/örnek/data/gutenberg/** için WASB klasöründe **/myfolder** Data Lake Store hesabındaki.
...
* Toplam YARN bellek - gelen Ambari portal YARN bellek D14 düğümü için 96 GB olduğunu belirler. Bu nedenle, 4 düğümlü bir küme için toplam YARN bellek şöyledir: 
