...
3.  Klasik Azure portalında kaynak adını almak ve [, StorSimple Yöneticisi hizmetiniz için kayıt anahtarını](storsimple-deployment-walkthrough-u2.md#step-2-get-the-service-registration-key).
4.  PowerShell Betiği çalıştıran sunucuda yönetici olarak çalıştırın. Bu komutu yazın:

    `Get-AzureStorSimpleDeviceBackupPolicy –DeviceName <device name>`

    Yedekleme İlkesi kimliğini not alın.
5.  Not Defteri'nde, aşağıdaki kodu kullanarak yeni bir PowerShell Betiği oluşturun.
    ...
    ```
      Yayımlama ayarları PowerShell komut dosyası Azure kaydettiğiniz aynı konuma kaydedin. Örneğin, C:\CloudSnapshot\StorSimpleCloudSnapshot.ps1 kaydedin.
6.  Betik yedekleme, yedekleme Exec işinde Backup Exec iş seçeneklerinizi ön işleme düzenleme ve sonrası Komutlar işleme ekleyin.
