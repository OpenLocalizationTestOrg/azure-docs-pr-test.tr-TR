...
Microsoft Azure Media Services, toodeliver (dinamik) Gelişmiş Şifreleme Standardı ((128-bit şifreleme anahtarları kullanılarak) AES) ve PlayReady veya Widevine DRM ile şifrelenmiş içeriğinizi sağlar. Media Services de tooauthorized istemcileri anahtarları ve PlayReady/Widevine lisansları teslim etmek için bir hizmet sunar.
...
* toobe mümkün toouse dinamik paketleme ve dinamik şifreleme akış içeriğinizi toostream istediğiniz uç hello hello olduğundan emin olun **çalıştıran** durumu.
...
> [!NOTE]
> Merhaba Media Services REST API ile çalışma hello olduğunda aşağıdaki maddeler geçerlidir:
> 
...
#### <a id="AddAuthorizationPolicyToKey"></a>Yetkilendirme İlkesi toohello içerik anahtarı Ekle
İsteği:
...
"Belirteç kısıtlama ilkesi" Merhaba gösterildiği gibi oluşturun [burada](#ContentKeyAuthorizationPolicies).
...
#### <a name="add-authorization-policy-toohello-content-key"></a>Yetkilendirme İlkesi toohello içerik anahtarı Ekle
AuthorizationPolicy toohello ContentKey gösterildiği gibi ekleyin [burada](#AddAuthorizationPolicyToKey).
...
İçeriğinizi PlayReady ile korurken hello özelliklerinden biri ihtiyacınız toospecify Yetkilendirme İlkesi'nde hello tanımlayan bir XML dizesi olan [PlayReady lisans şablonu](media-services-playready-license-template-overview.md). 
...
İçerik anahtarının yetkilendirme ilkesini yapılandırmış, toohello Git [nasıl tooconfigure varlık teslim ilkesini](media-services-rest-configure-asset-delivery-policy.md) konu.
