...
Bu anlatımda gösterilir nasıl sipariş toohandle artıklık isteğe bağlı akış için başka bir varlığa toocopy içeriği (BLOB). Bu senaryo, Azure içerik teslim ağı toofail yukarı tooset üzerinden bir veri merkezinde bir kesinti durumunda iki veri merkezi arasında istiyorsanız yararlıdır. Bu kılavuzda hello Azure Media Services SDK'sı, hello Azure Media Services REST API ve görevleri aşağıdaki hello Azure depolama SDK'sı toodemonstrate hello kullanılır:

1. Bir Media Services hesabı "Veri merkezi A'daki" ayarlama
2. Bir ara dosyayı bir kaynak varlığa yükleyin.
3. Merhaba varlık Çoklu bit hızı MP4 dosyasına kodlayın. 
4. Bir salt okunur paylaşılan erişim imzası Bulucu oluşturun. Bu, hello kaynak varlık toohave okuma erişimi toohello kapsayıcısında hello kaynak varlık ile ilişkili hello depolama hesabı içindir.
5. Merhaba kaynak varlık Hello kapsayıcı adı hello önceki adımda oluşturduğunuz hello salt okunur paylaşılan erişim imzası Bulucu alırlar. Bu (Merhaba konunun ilerleyen bölümlerinde açıklanmıştır) depolama hesapları arasında BLOB kopyalama için gereklidir
6. Görev kodlama hello tarafından oluşturulan hello varlık için bir kaynak Bulucu oluşturun. 

Ardından, toohandle hello yük devretme:

1. Bir Media Services hesabı "Veri merkezi b" olarak ayarlayın
2. Hedef boş varlık hello hedef Media Services hesabı oluşturun.
3. Bir yazma paylaşılan erişim imzası Bulucu oluşturun. Bu, hello hedef boş varlık toohave yazma erişimi toohello kapsayıcısında hello hedef varlıkla ilişkilidir hello hedef depolama hesabı içindir.
4. Merhaba kaynak depolama hesabı "Veri merkezi a" ve "Veri merkezi B'deki" Merhaba hedef depolama hesabı arasında Hello Azure depolama SDK'sı toocopy BLOB'lar (varlık dosyaları) kullanma Bu depolama hesapları ile ilgi hello varlıklar ilişkilendirilir.
5. BLOB'ları (varlık dosyaları) olan kopyalanan toohello hedef blob kapsayıcısı hello hedef varlık ile ilişkilendirin. 
6. "Veri merkezi b" Merhaba varlık için bir kaynak Bulucu oluşturun ve "Veri merkezi A'daki" Merhaba varlık için oluşturulan hello Bulucu kimliği belirtin

Merhaba göreli yollar hello URL'lerin nerede akış URL'leri hello kazandırır aynı (yalnızca hello temel URL'lerin farklıdır) hello. 

Ardından, toohandle herhangi kesinti bu kaynak konum belirleyicisi üstünde içerik teslim ağı oluşturabilirsiniz. 

ilgili önemli noktalar aşağıdaki hello Uygula:

* Media Services SDK'ın geçerli sürümü Hello bir varlığın varlık dosyaları ile ilişkilendirirsiniz program aracılığıyla oluşturma IAssetFile bilgi desteklemez. Bunun yerine, hello CreateFileInfos Media Services REST API toodo bunu kullanın. 
* (Merhaba şifreleme anahtarı hem de Media Services hesaplarını farklı olduğu için) şifrelenmiş depolama varlıklar (AssetCreationOptions.StorageEncrypted) için çoğaltma desteklenmiyor. 
* Dinamik paketleme tootake avantajlarından istiyorsanız, akış içeriğinizi toostream istediğiniz uç hello hello olduğundan emin olun **çalıştıran** durumu.

> [!NOTE]
> Merhaba Media Services kullanmayı [çoğaltıcı aracı](http://replicator.codeplex.com/) bir alternatif tooimplementing bir yük devretme senaryosu el ile akış olarak. Bu araç tooreplicate varlıklar arasında iki Media Services hesap sağlar.
> 
> 

## <a name="prerequisites"></a>Ön koşullar
* Yeni veya var olan bir Azure aboneliği iki Media Services hesabı. Bkz: [nasıl tooCreate Media Services hesabı](media-services-portal-create-account.md).
...
1. Visual Studio toocreate hello C# konsol uygulaması projesi içeren yeni bir çözüm kullanın. Girin **HandleRedundancyForOnDemandStreaming** hello adı ve ardından **Tamam**.
2. Merhaba oluşturma **SupportFiles** hello klasörü, aynı düzeydeki hello **HandleRedundancyForOnDemandStreaming.csproj** proje dosyası. Merhaba altında **SupportFiles** klasörünü hello oluşturma **OutputFiles** ve **MP4Files** klasörler. Merhaba bir .mp4 dosya kopyalama **MP4Files** klasör. (Bu örnekte, hello **BigBuckBunny.mp4** dosyası kullanılır.) 
3. Kullanım **Nuget** tooadd başvuruları tooDLLs ilgili tooMedia Hizmetleri. İçinde **Visual Studio ana menü**seçin **Araçları** > **kitaplık Paket Yöneticisi** > **Paket Yöneticisi Konsolu**. Merhaba konsol penceresine **Install-Package windowsazure.mediaservices**, ve Enter tuşuna basın.
4. Bu proje için gerekli olan diğer başvurular ekleyin: System.Configuration, System.Runtime.Serialization ve System.Web.
5. Değiştir **kullanarak** toohello eklenen deyimleri **Programs.cs** dosya olanları aşağıdaki hello ile varsayılan ayar:
...
6. Merhaba eklemek **appSettings** bölüm toohello **.config** dosya ve güncelleştirme hello değerleri anahtar ve ad değerleri tabanlı Media Services ve depolama alanı üzerinde. 
...
Bu bölümde, hello özelliği toohandle artıklık oluşturun.

1. Aşağıdaki sınıf düzeyi alanları toohello Program sınıfı hello ekleyin.
...
2. Merhaba varsayılan ana yöntem tanımını bir aşağıdaki hello ile değiştirin. Main adlı yöntemi tanımları aşağıda tanımlanmıştır.
...
3. yöntem tanımlarını aşağıdaki hello Main denir.

    >[!NOTE]
    >1.000.000 ilkelerinin farklı Media Services ilkeleri (örneğin, Bulucu ilke veya ContentKeyAuthorizationPolicy) için bir sınır yoktur. Merhaba kullanması gereken her zaman kullanıyorsanız, aynı ilke kimliği hello aynı gün ve erişim izinleri. Örneğin, aynı kimliği hello yerinde hedeflenen tooremain uzun bir süre (karşıya yükleme olmayan ilkeleri) olan bulucular için ilkeleri kullanın. Daha fazla bilgi için bkz: [bu konuda](media-services-dotnet-manage-entities.md#limit-access-policies).
...
Şimdi bir trafik Yöneticisi tooroute istekleri hello iki veri merkezi arasında kullanır ve böylece üzerinden herhangi kesinti durumunda başarısız.
