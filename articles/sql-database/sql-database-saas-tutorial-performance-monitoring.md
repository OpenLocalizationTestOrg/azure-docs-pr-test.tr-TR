...
---
# <a name="monitor-performance-of-the-wingtip-saas-application"></a>Wingtip SaaS uygulama performansını izleme
...
1. İçinde [Azure portal](https://portal.azure.com) açmak **Pool1**.
...
1. İnceleme **esnek veritabanı izleme** son bir saat içindeki yeni veritabanları gösteren görüntü. *Contosoconcerthall* veritabanı yakında görünmelidir beş yeni veritabanları biri olarak.
1. **Esnek veritabanı izleme'yi tıklatın** **grafik** ve açar **veritabanı kaynak kullanımını** burada izleyebilirsiniz herhangi bir veritabanı sayfası. Bu görüntü için ayırmanıza olanak tanır *contosoconcerthall* veritabanı.
1. Veritabanları listesinden tıklatın **contosoconcerthall**.
1. Tıklatın **fiyatlandırma Katmanı (Ölçek Dtu'lar)** açmak için **performansını yapılandırın** veritabanı için bir tek başına performans düzeyi ayarlayabileceğiniz sayfa.
1. Standart katmanındaki ölçeklendirme seçeneklerini açmak için **Standart** sekmesine tıklayın.
1. Slayt **DTU kaydırıcı** seçmek için sağdaki **100** Dtu'lar. Bu hizmet hedefi için karşılık gelen Not **S3**.
1. Tıklatın **Uygula** dışında havuzu veritabanını taşımak ve bunu yapmak için bir *standart S3* veritabanı.
1. Ölçeklendirme işlemi tamamlandıktan sonra contosoconcerthall veritabanı üzerindeki etkisini ve esnek havuzu ve veritabanı dikey pencereleri üzerinde Pool1 izleyin.
