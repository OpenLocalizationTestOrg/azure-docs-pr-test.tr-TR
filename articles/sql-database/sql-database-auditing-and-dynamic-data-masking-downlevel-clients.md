...
> [!IMPORTANT]
> Bu belge, yalnızca tooTable denetim geçerlidir **artık kullanım dışı**.<br>
> Lütfen hello yeni kullanın [Blob denetimi](sql-database-auditing.md) yöntemi, hangi **yok** alt düzey istemci bağlantı dizesi değişiklikler gerektirir. Blob denetimi hakkında ek bilgiler bulunabilir [SQL veritabanı denetimi ile çalışmaya başlama](sql-database-auditing.md).

[Veritabanı denetim](sql-database-auditing.md) otomatik olarak çalışır TDS yeniden yönlendirmeyi destekleyen SQL istemcileri ile. Merhaba Blob denetimi yöntemi kullanırken, yeniden yönlendirme uygulanmaz unutmayın.
...
Tablo denetim etkinleştirdiğinizde, hello IP uç veritabanınızın değiştirecek unutmayın. Katı güvenlik duvarı ayarları varsa, lütfen bu güvenlik duvarı ayarlarını güncelleştirmeniz buna göre.

Merhaba yeni veritabanı IP uç hello veritabanı bölgesinde bağlıdır:
