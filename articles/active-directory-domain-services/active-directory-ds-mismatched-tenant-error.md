...
Bu nedenle, burada yönetilen etki alanı ve etkin olarak sanal ağ ait iki farklı senaryolarda Azure AD kiracılarıyla bu hataya bakın.
...
- Tıklatın **silmek** varolan silmek için düğmesini yönetilen etki alanı. Kullanarak yeniden oluşturun [Azure portal](https://portal.azure.com), böylece yönetilen etki alanı ve sanal ağ içinde çıktığında Azure AD dizinine ait. Yeni oluşturulan yönetilen etki alanına daha önce silinmiş etki alanına katılan tüm makineler afresh katılması gerekir.

- Yönetilen etki alanınızı ait olduğu Azure AD dizini sanal ağa içeren Azure aboneliği taşımak için Azure desteğine başvurun. Tıklatın **yeni destek isteği** ve belirtin **eşleşmeyen dizin** içinde **ayrıntıları** destek isteği bölümü. Hata iletisinde destek isteğinin bir parçası olarak sağlanan bilgileri içerir.
