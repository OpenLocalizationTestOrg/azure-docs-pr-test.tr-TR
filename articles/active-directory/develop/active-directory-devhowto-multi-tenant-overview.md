...
Çoğu kuruluş için bir hizmet uygulaması olarak bir yazılım teklifi sunuyorsanız, uygulamanızın oturum açmalardan tüm Azure AD kiracısı kabul edecek şekilde yapılandırabilirsiniz.  Azure AD'de bu uygulama birden çok Kiracı yapmadan çağrılır.  Tüm Azure AD Kiracı kullanıcılar hesaplarına uygulamanızla birlikte kullanmak için onaylıyorsunuz sonra uygulamanıza oturum açabilir.  
...
Uygulama oturum açma yanıtı kullanıcıyı temsil eden bir belirteci içeriyor.  Belirteç veren değerinde bir uygulama kullanıcının arasındadır hangi Kiracı söyler.  Bir yanıt / Common döndüğünde uç noktası, Belirteç Verenin değeri karşılık kullanıcının Kiracı.  / Common dikkate almak önemlidir uç noktası bir kiracı değil ve bir veren değil, yalnızca çoğaltıcı değildir.  / Common kullanırken belirteçleri doğrulamak için uygulamanızın mantık bu dikkate güncelleştirilmesi gerekiyor. 
...
Tek kiracılı uygulama bir belirteci doğrular, meta veri belgesi İmzalama anahtarları karşı belirtecinin imzası denetler. Bu meta veri belgesinde bulunamadı bir belirteci veren değeriyle eşleştiğinden emin olmak için sağlar.
...
Belirli izinlere temsilci, ayrıca bir kiracı yönetici izni gerektirir.  Örneğin, oturum açmış olan kullanıcının olarak Azure AD ile geri yazma özelliğini bir kiracı yönetici izni gerektirir.  Yönetici izni gerektiren bir temsilci izni isteklerini bir uygulamaya oturum açmak normal bir kullanıcı çalışırsa, yalnızca uygulama izinleri gibi uygulamanızı hata iletisi alırsınız.  İzin gerektirip gerektirmediğini yönetici onayı kaynak yayımlanan geliştirici tarafından belirlenir ve kaynak belgelerinde bulunabilir.  Azure AD grafik API'si ve Microsoft Graph API için izinleri açıklayan konulara bağlantılar [ilgili içerik](#related-content) bu makalenin.
...
`prompt=admin_consent` Parametresi da yönetici onayı gerektirmeyen izinleri gerektiren uygulamalar tarafından kullanılabilir. Uygulama nerede Kiracı Yönetici "kaydolduğunda" bir zaman ve başka kullanıcılara izin o noktadan itibaren üzerinde sorulur bir deneyim gerektirdiğinde bu yapılır.

Bir uygulama yönetici izni gerektirir ve bir yönetici oturum açtığında, ancak `prompt=admin_consent` parametresi gönderilmez, yönetici uygulamaya başarıyla onayı **yalnızca kullanıcı hesapları için**.  Normal kullanıcı hala oturum açın ve uygulamaya onayı mümkün olmaz.  Bu, Kiracı yönetici diğer kullanıcılar erişime izin vermeden önce uygulamanızın keşfedin vermek istiyorsanız yararlıdır.
...
Lütfen aşağıdaki Açıklamalar bölümüne geri bildirim sağlamak ve iyileştirmek ve içeriği şekil yardımcı olmak için kullanın.
