...
Bu öğreticide, bilgi nasıl toointegrate Zscaler Beta Azure Active Directory'ye (Azure AD).

Zscaler Beta Azure AD ile tümleştirme ile Merhaba aşağıdaki avantajları sağlar:

- Erişim tooZscaler Beta sahip Azure AD'de kontrol edebilirsiniz
- Kullanıcıların tooautomatically get açan tooZscaler Beta (çoklu oturum açma) Azure AD hesaplarına sahip etkinleştirebilirsiniz
...
tooconfigure Zscaler Beta ile Azure AD tümleştirme, aşağıdaki öğelerindeki hello gerekir:
...
1. Merhaba Galerisi'nden Zscaler Beta ekleme
2. Çoklu oturum açmayı yapılandırma ve Azure AD sınama

## <a name="adding-zscaler-beta-from-hello-gallery"></a>Merhaba Galerisi'nden Zscaler Beta ekleme
Azure AD'ye tooconfigure hello tümleştirme Zscaler Beta'nin tooadd Zscaler Beta hello galeri tooyour listesinden yönetilen SaaS uygulamaları gerekir.

**tooadd Zscaler Beta hello galerisinden hello aşağıdaki adımları gerçekleştirin:**
...
4. Merhaba arama kutusuna yazın **Zscaler Beta**.
...
5. Merhaba Sonuçlar panelinde seçin **Zscaler Beta**ve ardından **Ekle** düğmesini tooadd Merhaba uygulaması.
...
Tek toowork'ın oturum açma hangi hello karşılık gelen Zscaler Beta tooa kullanıcı Azure AD içinde olduğu Azure AD tooknow gerekir. Diğer bir deyişle, bir Azure AD kullanıcısının ve Zscaler Beta hello ilgili kullanıcı arasında bir bağlantı ilişkisi kurulan toobe gerekir.

Merhaba hello değeri Zscaler Beta'da atayın **kullanıcı adı** hello hello değeri olarak Azure AD'de **kullanıcıadı** tooestablish hello bağlantı ilişkisi.

tooconfigure ve Zscaler Beta ile Azure AD çoklu oturum açmayı test, yapı taşları aşağıdaki toocomplete hello gerekir:
...
4. **[Zscaler Beta test kullanıcısı oluşturma](#creating-a-zscaler-beta-test-user)**  -toohave Britta Simon kullanıcı bağlantılı toohello Azure AD gösterimidir Zscaler Beta, karşılık gelen.
...
Bu bölümde, Azure AD çoklu oturum açma hello Azure portal'ın etkinleştirin ve çoklu oturum açma Zscaler Beta uygulamanızda yapılandırın.

**tooconfigure Azure AD çoklu oturum açma Zscaler Beta ile Merhaba aşağıdaki adımları gerçekleştirin:**

1. Hello hello üzerinde Azure portal'ın **Zscaler Beta** uygulama tümleştirme sayfasını tıklatın **çoklu oturum açma**.
...
3. Merhaba üzerinde **Zscaler Beta etki alanı ve URL'leri** bölümünde, hello aşağıdaki adımları gerçekleştirin:

    ![Çoklu oturum açmayı yapılandırın](./media/active-directory-saas-zscaler-beta-tutorial/tutorial_zscalerbeta_url.png)

    Merhaba oturum açma URL'si metin kutusuna, kullanıcıların toosign üzerinde tooyour tarafından Zscaler Beta uygulama kullanılan hello URL'sini yazın.

    > [!NOTE] 
    > Bu değeri hello ile tooupdate sahip gerçek oturum açma URL'si. Kişi [Zscaler Beta istemci destek ekibi](https://www.zscaler.com/company/contact) tooget bu değer. 
...
6. Merhaba üzerinde **Zscaler Beta yapılandırma** 'yi tıklatın **yapılandırma Zscaler Beta** tooopen **yapılandırma oturum açma** penceresi. Kopya hello **SAML çoklu oturum açma hizmet URL'si** hello gelen **hızlı başvuru bölümü.**

    ![Çoklu oturum açmayı yapılandırın](./media/active-directory-saas-zscaler-beta-tutorial/tutorial_zscalerbeta_configure.png) 

7. Farklı web tarayıcısı penceresinde tooyour içinde Zscaler Beta şirket site yönetici olarak oturum.
...
5. Hello Proxy sunucu bölümüne, hello aşağıdaki adımları gerçekleştirin:   
    ...
    b. Başlangıç adresi metin kutusuna yazın **gateway.zscalerbeta.net**.
...
tooenable Azure AD kullanıcıların toolog tooZscaler Beta'da, sağlanan tooZscaler Beta olması gerekir. Zscaler Beta Hello durumda sağlama bir el ile bir görevdir.
...
1. İçinde tooyour oturum **Zscaler Beta** Kiracı.
...
> [!NOTE]
> API'leri, Azure AD kullanıcı hesapları Zscaler Beta tooprovision tarafından sağlanan veya herhangi diğer Zscaler Beta kullanıcı hesabı oluşturma araçlarını kullanabilirsiniz.

### <a name="assigning-hello-azure-ad-test-user"></a>Hello Azure AD test kullanıcısı atama

Bu bölümde, erişim tooZscaler Beta vererek Britta Simon toouse Azure çoklu oturum açmayı etkinleştirin.

![Kullanıcı atama][200] 

**tooassign Britta Simon tooZscaler Beta hello aşağıdaki adımları gerçekleştirin:**

1. Hello Azure portal, hello uygulamaları görünümü Aç ve ardından toohello dizin görünümüne gidin ve çok Git**kurumsal uygulamalar** ardından **tüm uygulamaları**.

    ![Kullanıcı atama][201] 

2. Merhaba uygulamalar listesinde **Zscaler Beta**.
...
Zscaler Beta döşeme hello erişim paneli hello tıkladığınızda, otomatik olarak oturum açma Zscaler Beta uygulama tooyour almanız gerekir.
