...
Ayrıcalıkları Etkilenme süresini azaltmak ve bunların kullanılması, görünürlük artırmak için yalnızca "tam zamanında" ayrıcalıklarını üzerinde alan kullanıcıların sınırla (JIT), bir görev gerçekleştirmeniz gerektiğinde. Azure Active Directory ve Microsoft Online Services için kullanabileceğiniz [Azure AD Privileged Identity Management (PIM)](http://aka.ms/AzurePIM).
